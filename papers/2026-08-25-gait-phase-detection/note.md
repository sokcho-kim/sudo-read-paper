...
Dataset.zip 2.0 GB 전체 확보, Post CSV 288개 전수 파싱 (`verify/audit_dataset.py`, 결과 `verify/audit_result.json`·`audit_detail.json`).
...
- 이벤트가 하나라도 있는 파일: **288개 중 ~86개(30%)**. HP112는 **0개**, HP114는 1개뿐. 있는 것도 HP115/116/117 forward와 UP112에 집중.
- 전체 오른발 HC 이벤트 953개, TO 782개. 보행 모드 샘플 293만 개 ÷ 주기당 ~100샘플 ≈ **예상 보행주기 약 29,000개 대비 ~3% 커버리지**.
